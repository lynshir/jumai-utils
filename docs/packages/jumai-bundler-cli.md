...
mkdir 目录名称
cd 目录名称
npx -p yo@4.1.0 -p generator-jumai-react-h5 -c "yo egenie-react-h5"
```
...
mkdir 目录名称
cd 目录名称
npx -p yo -p generator-jumai-react-web -c "yo egenie-react-web"
```
...
```shell
jumai-bundler-cli dev --vite
```

### dev 以 webpack 模式启动

```shell
jumai-bundler-cli dev
```

### webpack 打包

```shell
jumai-bundler-cli build
```
...
```json
{
  ...
  "useBuiltIns": "usage",
  "corejs": {
    "version": "jumai-babel-preset包对应core/js版本",
    "proposals": true
  },
...
```json
{
  "helpers": true,
  "regenerator": true,
  "absoluteRuntime": "jumai-babel-preset包对应的路径",
  "version": "jumai-babel-preset包@babel/runtime的版本"
}
```
