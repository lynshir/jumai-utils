...
mkdir 目录名称
cd 目录名称
npx -p yo@4.1.0 -p generator-egenie-react-h5 -c "yo egenie-react-h5"
```
...
mkdir 目录名称
cd 目录名称
npx -p yo -p generator-egenie-react-web -c "yo egenie-react-web"
```
...
```shell
egenie-bundler-cli dev --vite
```

### dev 以 webpack 模式启动

```shell
egenie-bundler-cli dev
```

### webpack 打包

```shell
egenie-bundler-cli build
```
...
```json
{
  ...
  "useBuiltIns": "usage",
  "corejs": {
    "version": "egenie-babel-preset包对应core/js版本",
    "proposals": true
  },
...
```json
{
  "helpers": true,
  "regenerator": true,
  "absoluteRuntime": "egenie-babel-preset包对应的路径",
  "version": "egenie-babel-preset包@babel/runtime的版本"
}
```
...
- 描述: px 转 rem(`h5项目用,脚手架模板初始化37.5,根据实际修改`)
- 类型: { remUnit?: number; } & Record<string, any>;
- 默认值: null
- 兼容模式: vite 和 webpack
